...
// Description of |722. Remove Comments|:
//
// Given a C++ program, remove comments from it. The program source is an array
// of strings |source| where |source[i]| is the |iᵗʰ| line of the source code.
...
LEETCODE_BEGIN_RESOLVING(722, RemoveComments, Solution);
    ...
    for (int i = 0; i < source.size(); i++) {
      for (int j = 0; j < source[i].size(); j++) {
        if (!comment && j + 1 < source[i].size() && source[i][j] == '/' &&
          ...
          comment = true;
          j++;
        } else if (comment && j + 1 < source[i].size() && source[i][j] == '*' &&
                   source[i][j + 1] == '/') {
          comment = false;
          j++;
        } else if (!comment) {
...
// string // denotes line 4 as comments. The line by line output code is
// visualized as below: int main()
// {
//
// int a, b, c;
// a = b + c;
...
LEETCODE_SOLUTION_UNITTEST(722, RemoveComments, example_1) {
  ...
  vector<string> expect   = {"int main()",   "{ ",         " ",
                             "int a, b, c;", "a = b + c;", "}"};
  vector<string> actual   = solution->removeComments(source);
...
// have bolded the newline characters. After deletion, the implicit newline
// characters are deleted, leaving the string "ab", which when delimited by
// newline characters becomes ["ab"].

LEETCODE_SOLUTION_UNITTEST(722, RemoveComments, example_2) {
  auto           solution = MakeSolution();
